    ...
    function fetch($page = 1){
        try{
            ...
            if(\Auth::user()->role_id == 2){
                $orders = Order::with('status', 'car', 'user', 'client', 'payments')->has('client')->where('status_id', '>', 3)->skip($skip)->take(15)->orderBy('id', 'desc')->get();
                $ordersCount = Order::with('status', 'car', 'user', 'client', 'payments')->has('client')->where('status_id', '>', 3)->count();
            }

            else if(\Auth::user()->role_id == 3){
                $orders = Order::with('status', 'car', 'user', 'client', 'payments')->has('client')->where('status_id', '>=', 1)->where('status_id', '<', 3)->skip($skip)->take(15)->orderBy('id', 'desc')->get();
                $ordersCount = Order::with('status', 'car', 'user', 'client', 'payments')->has('client')->where('status_id', '>=', 1)->where('status_id', '<', 3)->count();
            }

            else{
                $orders = Order::with('status', 'car', 'user', 'client', 'payments', 'mechanic')->has('client')->skip($skip)->take(15)->orderBy('id', 'desc')->get();
                $ordersCount = Order::with('status', 'car', 'user', 'client', 'payments', 'mechanic')->has('client')->count();
            }
